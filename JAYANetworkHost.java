 ...
 */
public class JAYANetworkHost extends Host {
	...
	public JAYAEdgeSwitch sw; // Edge switch in general
	...
	private void recvpackets() {
		...
		for (NetworkPacket hs : packetrecieved) {
			...
			List<HostPacket> pktlist = ((JAYANetworkCloudletSpaceSharedScheduler) vm.getCloudletScheduler()).pktrecv
					.get(hs.pkt.sender);

			if (pktlist == null) {
				pktlist = new ArrayList<HostPacket>();
				((JAYANetworkCloudletSpaceSharedScheduler) vm.getCloudletScheduler()).pktrecv.put(
						hs.pkt.sender,
						pktlist);
	...
	private void sendpackets() {
		...
		for (Vm vm : super.getVmList()) {
			for (Entry<Integer, List<HostPacket>> es : ((JAYANetworkCloudletSpaceSharedScheduler) vm
		...
		for (NetworkPacket hs : packetTosendLocal) {
			...
			JAYANetdatacenterBroker.totalTransferCost += hs.pkt.data * JAYANetdatacenterBroker.transferCost[hs.pkt.sender][hs.pkt.reciever];
