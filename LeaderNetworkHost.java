 ...
 */
public class LeaderNetworkHost extends Host {
	...
	public LeaderSwitch sw; // Edge switch in general
	...
	private void recvpackets() {
		...
		for (NetworkPacket hs : packetrecieved) {
			...
			List<HostPacket> pktlist = ((LeaderNetworkCloudletSpaceSharedScheduler) vm.getCloudletScheduler()).pktrecv
					.get(hs.pkt.sender);

			if (pktlist == null) {
				pktlist = new ArrayList<HostPacket>();
				((LeaderNetworkCloudletSpaceSharedScheduler) vm.getCloudletScheduler()).pktrecv.put(
						hs.pkt.sender,
						pktlist);
	...
	private void sendpackets() {
		...
		for (Vm vm : super.getVmList()) {
			for (Entry<Integer, List<HostPacket>> es : ((LeaderNetworkCloudletSpaceSharedScheduler) vm
		...
		for (NetworkPacket hs : packetTosendLocal) {
			...
			Leader_NetdatacenterBroker.totalTransferCost += hs.pkt.data * Leader_NetdatacenterBroker.transferCost[hs.pkt.sender][hs.pkt.reciever];
