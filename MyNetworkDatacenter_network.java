...
public class MyNetworkDatacenter_network extends NetDatacenterBroker{
	 ...
	 * @post $none
	 */
	public MyNetworkDatacenter_network(String name) throws Exception {
		super(name);
	...
	protected void createVmAndWorkflow(int datacenterId) {
		...
		*/
		...
		eamScheduling = new IEAM(workflowDataTransferMap, workflowExecutionMI, linkDC, numberOfTasks);
		ArrayList<Integer> taskToVmMap = eamScheduling.runIEAM();
	...
	private void CreateVMs2(int datacenterId) {
		...
		for (int i = 0; i < numVM; i++) {
			...
			NetworkVm vm = new NetworkVm(vmid, getId(), mips, pesNumber, ram,
					bw, size, vmm, new NetworkCloudletSpaceSharedScheduler());
			linkDC.processVmCreateNetwork(vm);
			// add the VM to the vmList
			getVmList().add(vm);
