 ...
 */
public class TLBONetworkCloudletSpaceSharedScheduler extends CloudletScheduler {
	 ...
	 * @post $none
	 */
	public TLBONetworkCloudletSpaceSharedScheduler() {
		super();
			...
			if ((cl.currStagenum != -1)) {
				...
				if (st.type == NetworkConstants.EXECUTION) {
					...
					if (cl.timespentInStage >= st.time) {
						...
						TLBONetdatacenterBroker.totalExeCost += cl.timespentInStage * TLBONetdatacenterBroker.executionCost[cl.getVmId()];
						//Leader_NetdatacenterBroker.totalExeCost += cl.timespentInStage * Leader_NetdatacenterBroker.executionCost[cl.getVmId()];
				...
				if (cl.stages.get(0).type == NetworkConstants.EXECUTION) {
					TLBONetdatacenterBroker.linkDC.schedule(
							TLBONetdatacenterBroker.linkDC.getId(),
							cl.stages.get(0).time,
							CloudSimTags.VM_DATACENTER_EVENT);
					//Leader_NetdatacenterBroker.linkDC.schedule(Leader_NetdatacenterBroker.linkDC.getId(), cl.stages.get(0).time,CloudSimTags.VM_DATACENTER_EVENT);
					...
					//Leader_NetdatacenterBroker.linkDC.schedule(Leader_NetdatacenterBroker.linkDC.getId(),0.0001,CloudSimTags.VM_DATACENTER_EVENT);
	...
	private void changetonextstage(NetworkCloudlet cl, TaskStage st) {
		...
		if (currstage >= (cl.stages.size() - 1)) {
			...
			TLBONetdatacenterBroker.linkDC.schedule(
					...
					0.0001,
					CloudSimTags.VM_DATACENTER_EVENT);
			/*Leader_NetdatacenterBroker.linkDC.schedule(
					Leader_NetdatacenterBroker.linkDC.getId(),
					0.0001,
					CloudSimTags.VM_DATACENTER_EVENT);*/
			if (i == cl.stages.size()) {
				cl.currStagenum = NetworkConstants.FINISH;
