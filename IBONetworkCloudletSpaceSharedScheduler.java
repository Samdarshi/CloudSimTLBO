 ...
 */
public class IBONetworkCloudletSpaceSharedScheduler extends CloudletScheduler {
	 ...
	 * @post $none
	 */
	public IBONetworkCloudletSpaceSharedScheduler() {
		super();
			...
			if ((cl.currStagenum != -1)) {
				...
				if (st.type == NetworkConstants.EXECUTION) {
					...
					if (cl.timespentInStage >= st.time) {
						...
						IBONetDatacenterBroker.totalExeCost += cl.timespentInStage * IBONetDatacenterBroker.executionCost[cl.getVmId()];
						//JAYANetdatacenterBroker.totalExeCost += cl.timespentInStage * JAYANetdatacenterBroker.executionCost[cl.getVmId()];
				...
				if (cl.stages.get(0).type == NetworkConstants.EXECUTION) {
					IBONetDatacenterBroker.linkDC.schedule(
							IBONetDatacenterBroker.linkDC.getId(),
							cl.stages.get(0).time,
							CloudSimTags.VM_DATACENTER_EVENT);
					//JAYANetdatacenterBroker.linkDC.schedule(JAYANetdatacenterBroker.linkDC.getId(), cl.stages.get(0).time,CloudSimTags.VM_DATACENTER_EVENT);
					...
					//JAYANetdatacenterBroker.linkDC.schedule(JAYANetdatacenterBroker.linkDC.getId(),0.0001,CloudSimTags.VM_DATACENTER_EVENT);
	...
	private void changetonextstage(NetworkCloudlet cl, TaskStage st) {
		...
		if (currstage >= (cl.stages.size() - 1)) {
			...
			IBONetDatacenterBroker.linkDC.schedule(
					...
					0.0001,
					CloudSimTags.VM_DATACENTER_EVENT);
			/*JAYANetdatacenterBroker.linkDC.schedule(
					JAYANetdatacenterBroker.linkDC.getId(),
					0.0001,
					CloudSimTags.VM_DATACENTER_EVENT);*/
			if (i == cl.stages.size()) {
				cl.currStagenum = NetworkConstants.FINISH;
