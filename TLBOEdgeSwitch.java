...
/**
 * This class allows to simulate Edge TLBOSwitch for Datacenter network. It interacts with other
 ...
 */
public class TLBOEdgeSwitch extends TLBOSwitch {


	/**
	 * Constructor for Edge TLBOSwitch We have to specify switches that are connected to its downlink
	 * and uplink ports, and corresponding bandwidths. In this TLBOSwitch downlink ports are connected
	 * to hosts not to a TLBOSwitch.
	 * 
	 * @param name Name of the TLBOSwitch
	 * @param level At which level TLBOSwitch is with respect to hosts.
	 * @param dc Pointer to Datacenter
	 */
	public TLBOEdgeSwitch(String name, int level, TLBONetworkDatacenter dc) {
		super(name, level, dc);
		hostlist1 = new HashMap<Integer, TLBONetworkHost>();
		...
		uplinkswitches = new ArrayList<TLBOSwitch>();
	}

	/**
	 * Send Packet to TLBOSwitch connected through a uplink port
	 * 
	...
	protected void processpacket_up(SimEvent ev) {
		...
		// has to send up
		// check which TLBOSwitch to forward to
		// add packet in the TLBOSwitch list
		//
		...
		TLBONetworkHost hs = hostlist1.get(hostid);
		hspkt.recieverhostid = hostid;

		// packet needs to go to a host which is connected directly to TLBOSwitch
		if (hs != null) {
			// packet to be sent to host connected to the TLBOSwitch
		...
		// otherwise
		// packet is to be sent to upper TLBOSwitch
		// ASSUMPTION EACH EDGE is Connected to one aggregate level TLBOSwitch
		// if there are more than one Aggregate level TLBOSwitch one need to modify following code

		TLBOSwitch sw = uplinkswitches.get(0);
	...
	/**
	 * Send Packet to hosts connected to the TLBOSwitch
	 * 
	 * @param ev Event/packet to process
	 */
	@Override
	protected void processpacketforward(SimEvent ev) {
		...
		// or to TLBOSwitch at next level.
		// clear the list
