...
public class LeaderSwitch extends SimEntity {
	...
	//public Map<Integer, LeaderNetworkHost> hostlist;
	
	public Map<Integer, LeaderNetworkHost> hostlist1;
	public List<LeaderSwitch> uplinkswitches;

	public List<LeaderSwitch> downlinkswitches;
	...
	public LeaderNetworkDatacenter dc;

	// something is running on these hosts
	//public SortedMap<Double, List<LeaderNetworkHost>> fintimelistHost = new TreeMap<Double, List<LeaderNetworkHost>>();
	public SortedMap<Double, List<LeaderNetworkHost>> fintimelistHost1 = new TreeMap<Double, List<LeaderNetworkHost>>();
	...
	public LeaderSwitch(String name, int level, LeaderNetworkDatacenter dc) {
		super(name);
		this.level = level;
		this.dc = dc;
	}
	...
	protected void processhostpacket(SimEvent ev) {
		...
		LeaderNetworkHost hs = hostlist1.get(hspkt.recieverhostid);
	...
	protected void processpacket_up(SimEvent ev) {
		...
		if (level == NetworkConstants.Agg_LEVEL) {
			...
			for (LeaderSwitch sw : downlinkswitches) {
		...
		if (level == NetworkConstants.ROOT_LEVEL) {
			...
			;
			for (LeaderSwitch sw : downlinkswitches) {
				for (LeaderSwitch edge : sw.downlinkswitches) {
	...
	private void registerHost(SimEvent ev) {
		LeaderNetworkHost hs = (LeaderNetworkHost) ev.getData();
		hostlist1.put(hs.getId(), (LeaderNetworkHost) ev.getData());
	}
	...
	protected LeaderNetworkHost getHostwithVM(int vmid) {
		for (Entry<Integer, LeaderNetworkHost> es : hostlist1.entrySet()) {
	...
	protected List<LeaderNetworkHost> getfreehostlist(int numhost) {
		List<LeaderNetworkHost> freehostls = new ArrayList<LeaderNetworkHost>();
		for (Entry<Integer, LeaderNetworkHost> et : hostlist1.entrySet()) {
