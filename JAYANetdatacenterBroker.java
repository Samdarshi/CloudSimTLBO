 ...
 */
public class JAYANetdatacenterBroker extends SimEntity {
	...
	public static JAYANetworkDatacenter linkDC;
	
	//private PSO psoScheduling;
	//private IEAM ieamScheduling;

	private JAYAEVOVL LeaderoScheduling;
	 ...
	 * @post $none
	 */
	public JAYANetdatacenterBroker(String name) throws Exception {
		super(name);
	...
	public void setLinkDC(JAYANetworkDatacenter datacenter0) {
		linkDC = datacenter0;
	}
	...
	protected void createVmAndWorkflow(int datacenterId) {
		...
		*/
		...
		LeaderoScheduling = new JAYAEVOVL(workflowDataTransferMap, workflowExecutionMI, linkDC, numberOfTasks);
		ArrayList<Integer> taskToVmMap = LeaderoScheduling.runJAYA();
	...
	private void CreateVMs(int datacenterId) {
		...
		for (int i = 0; i < numVM; i++) 
		{
			...
			NetworkVm vm = new NetworkVm(i, getId(), mips[i], pesNumber, ram,
					bw, size, vmm, new JAYANetworkCloudletSpaceSharedScheduler(), executionCost[i], 
					transferCost[i]);
