 ...
 */
public class TLBONetworkHost extends Host {
	...
	public TLBOSwitch sw; // Edge switch in general
	...
	private void recvpackets() {
		...
		for (NetworkPacket hs : packetrecieved) {
			...
			List<HostPacket> pktlist = ((TLBONetworkCloudletSpaceSharedScheduler) vm.getCloudletScheduler()).pktrecv
					.get(hs.pkt.sender);

			if (pktlist == null) {
				pktlist = new ArrayList<HostPacket>();
				((TLBONetworkCloudletSpaceSharedScheduler) vm.getCloudletScheduler()).pktrecv.put(
						hs.pkt.sender,
						pktlist);
	...
	private void sendpackets() {
		...
		for (Vm vm : super.getVmList()) {
			for (Entry<Integer, List<HostPacket>> es : ((TLBONetworkCloudletSpaceSharedScheduler) vm
		...
		for (NetworkPacket hs : packetTosendLocal) {
			...
			TLBONetdatacenterBroker.totalTransferCost += hs.pkt.data * TLBONetdatacenterBroker.transferCost[hs.pkt.sender][hs.pkt.reciever];
