...
public class Switch extends SimEntity {
	...
	//public Map<Integer, IEAMNetworkHost> hostlist;
	
	public Map<Integer, IEAMNetworkHost> hostlist1;
	...
	//public SortedMap<Double, List<IEAMNetworkHost>> fintimelistHost = new TreeMap<Double, List<IEAMNetworkHost>>();
	public SortedMap<Double, List<IEAMNetworkHost>> fintimelistHost1 = new TreeMap<Double, List<IEAMNetworkHost>>();
	...
	protected void processhostpacket(SimEvent ev) {
		...
		IEAMNetworkHost hs = hostlist1.get(hspkt.recieverhostid);
	...
	protected void processpacket_up(SimEvent ev) {
		...
		if (level == NetworkConstants.EDGE_LEVEL) {
			...
			int hostid = dc.VmtoHostlist.get(recvVMid);
			IEAMNetworkHost hs = hostlist1.get(hostid);
			hspkt.recieverhostid = hostid;
			if (hs != null) {
	...
	private void registerHost(SimEvent ev) {
		IEAMNetworkHost hs = (IEAMNetworkHost) ev.getData();
		hostlist1.put(hs.getId(), (IEAMNetworkHost) ev.getData());
	}
	...
	protected IEAMNetworkHost getHostwithVM(int vmid) {
		for (Entry<Integer, IEAMNetworkHost> es : hostlist1.entrySet()) {
	...
	protected List<IEAMNetworkHost> getfreehostlist(int numhost) {
		List<IEAMNetworkHost> freehostls = new ArrayList<IEAMNetworkHost>();
		for (Entry<Integer, IEAMNetworkHost> et : hostlist1.entrySet()) {
