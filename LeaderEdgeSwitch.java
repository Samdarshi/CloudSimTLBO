...
/**
 * This class allows to simulate Edge LeaderSwitch for Datacenter network. It interacts with other
 ...
 */
public class LeaderEdgeSwitch extends LeaderSwitch {


	/**
	 * Constructor for Edge LeaderSwitch We have to specify switches that are connected to its downlink
	 * and uplink ports, and corresponding bandwidths. In this LeaderSwitch downlink ports are connected
	 * to hosts not to a LeaderSwitch.
	 * 
	 * @param name Name of the LeaderSwitch
	 * @param level At which level LeaderSwitch is with respect to hosts.
	 * @param dc Pointer to Datacenter
	 */
	public LeaderEdgeSwitch(String name, int level, LeaderNetworkDatacenter dc) {
		super(name, level, dc);
		hostlist1 = new HashMap<Integer, LeaderNetworkHost>();
		...
		uplinkswitches = new ArrayList<LeaderSwitch>();
	}

	/**
	 * Send Packet to LeaderSwitch connected through a uplink port
	 * 
	...
	protected void processpacket_up(SimEvent ev) {
		...
		// has to send up
		// check which LeaderSwitch to forward to
		// add packet in the LeaderSwitch list
		//
		...
		LeaderNetworkHost hs = hostlist1.get(hostid);
		hspkt.recieverhostid = hostid;

		// packet needs to go to a host which is connected directly to LeaderSwitch
		if (hs != null) {
			// packet to be sent to host connected to the LeaderSwitch
		...
		// otherwise
		// packet is to be sent to upper LeaderSwitch
		// ASSUMPTION EACH EDGE is Connected to one aggregate level LeaderSwitch
		// if there are more than one Aggregate level LeaderSwitch one need to modify following code

		LeaderSwitch sw = uplinkswitches.get(0);
	...
	/**
	 * Send Packet to hosts connected to the LeaderSwitch
	 * 
	 * @param ev Event/packet to process
	 */
	@Override
	protected void processpacketforward(SimEvent ev) {
		...
		// or to LeaderSwitch at next level.
		// clear the list
