...
import org.cloudbus.cloudsim.network.datacenter.IEAMNetworkHost;
...
public class IEAMSchedulingTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			l.add(NetDatacenterBroker.totalExeCost);
	...
	private static NetworkDatacenter createDatacenter(String name) {
		...
		// our machine
		List<IEAMNetworkHost> hostList = new ArrayList<IEAMNetworkHost>();
		...
		for (int index = 0; index < hostNumber; index++) {
			hostList.add(new IEAMNetworkHost(index, new RamProvisionerSimple(ram),
	...
	static void CreateNetwork(int numhost, NetworkDatacenter dc) {
		...
		for (Host hs : dc.getHostList()) {
			IEAMNetworkHost hs1 = (IEAMNetworkHost) hs;
			...
			List<IEAMNetworkHost> hslist = hs1.sw.fintimelistHost1.get(0D);
			if (hslist == null) {
				hslist = new ArrayList<IEAMNetworkHost>();
