...
public class TLBOSwitch extends SimEntity {
	...
	//public Map<Integer, TLBONetworkHost> hostlist;
	
	public Map<Integer, TLBONetworkHost> hostlist1;
	public List<TLBOSwitch> uplinkswitches;

	public List<TLBOSwitch> downlinkswitches;
	...
	public TLBONetworkDatacenter dc;

	// something is running on these hosts
	//public SortedMap<Double, List<TLBONetworkHost>> fintimelistHost = new TreeMap<Double, List<TLBONetworkHost>>();
	public SortedMap<Double, List<TLBONetworkHost>> fintimelistHost1 = new TreeMap<Double, List<TLBONetworkHost>>();
	...
	public TLBOSwitch(String name, int level, TLBONetworkDatacenter dc) {
		super(name);
		this.level = level;
		this.dc = dc;
	}
	...
	protected void processhostpacket(SimEvent ev) {
		...
		TLBONetworkHost hs = hostlist1.get(hspkt.recieverhostid);
	...
	protected void processpacket_up(SimEvent ev) {
		...
		if (level == NetworkConstants.Agg_LEVEL) {
			...
			for (TLBOSwitch sw : downlinkswitches) {
		...
		if (level == NetworkConstants.ROOT_LEVEL) {
			...
			;
			for (TLBOSwitch sw : downlinkswitches) {
				for (TLBOSwitch edge : sw.downlinkswitches) {
	...
	private void registerHost(SimEvent ev) {
		TLBONetworkHost hs = (TLBONetworkHost) ev.getData();
		hostlist1.put(hs.getId(), (TLBONetworkHost) ev.getData());
	}
	...
	protected TLBONetworkHost getHostwithVM(int vmid) {
		for (Entry<Integer, TLBONetworkHost> es : hostlist1.entrySet()) {
	...
	protected List<TLBONetworkHost> getfreehostlist(int numhost) {
		List<TLBONetworkHost> freehostls = new ArrayList<TLBONetworkHost>();
		for (Entry<Integer, TLBONetworkHost> et : hostlist1.entrySet()) {
