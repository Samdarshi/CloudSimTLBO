...
public class IBOSwitch extends SimEntity {
	...
	public Map<Integer, IBONetworkHost> hostlist1;
	public List<IBOSwitch> uplinkswitches;

	public List<IBOSwitch> downlinkswitches;
	...
	public IBONetworkDatacenter dc;
	...
	public SortedMap<Double, List<IBONetworkHost>> fintimelistHost1 = new TreeMap<Double, List<IBONetworkHost>>();
	...
	public IBOSwitch(String name, int level, IBONetworkDatacenter dc2) {
		super(name);
		this.level = level;
		this.dc = dc2;
	}
	...
	protected void processhostpacket(SimEvent ev) {
		...
		IBONetworkHost hs = hostlist1.get(hspkt.recieverhostid);
	...
	protected void processpacket_up(SimEvent ev) {
		...
		if (level == NetworkConstants.EDGE_LEVEL) {
			...
			int hostid = dc.VmtoHostlist.get(recvVMid);
			IBONetworkHost hs = hostlist1.get(hostid);
			hspkt.recieverhostid = hostid;
			if (hs != null) {
			...
			IBOSwitch sw = uplinkswitches.get(0);
		...
		if (level == NetworkConstants.Agg_LEVEL) {
			...
			for (IBOSwitch sw : downlinkswitches) {
		...
		if (level == NetworkConstants.ROOT_LEVEL) {
			...
			;
			for (IBOSwitch sw : downlinkswitches) {
				for (IBOSwitch edge : sw.downlinkswitches) {
	...
	private void registerHost(SimEvent ev) {
		IBONetworkHost hs = (IBONetworkHost) ev.getData();
		hostlist1.put(hs.getId(), (IBONetworkHost) ev.getData());
	}
	...
	protected IBONetworkHost getHostwithVM(int vmid) {
		for (Entry<Integer, IBONetworkHost> es : hostlist1.entrySet()) {
	...
	protected List<IBONetworkHost> getfreehostlist(int numhost) {
		List<IBONetworkHost> freehostls = new ArrayList<IBONetworkHost>();
		for (Entry<Integer, IBONetworkHost> et : hostlist1.entrySet()) {
