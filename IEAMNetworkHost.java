 ...
 */
public class IEAMNetworkHost extends Host {
	...
	private void recvpackets() {
		...
		for (NetworkPacket hs : packetrecieved) {
			...
			List<HostPacket> pktlist = ((IEAMNetworkCloudletSpaceSharedScheduler) vm.getCloudletScheduler()).pktrecv
					.get(hs.pkt.sender);

			if (pktlist == null) {
				pktlist = new ArrayList<HostPacket>();
				((IEAMNetworkCloudletSpaceSharedScheduler) vm.getCloudletScheduler()).pktrecv.put(
						hs.pkt.sender,
						pktlist);
	...
	private void sendpackets() {
		...
		for (Vm vm : super.getVmList()) {
			for (Entry<Integer, List<HostPacket>> es : ((IEAMNetworkCloudletSpaceSharedScheduler) vm
