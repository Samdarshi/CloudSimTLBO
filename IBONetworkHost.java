 ...
 */
public class IBONetworkHost extends Host {
	...
	public IBOEdgeSwitch sw; // Edge switch in general
	...
	private void recvpackets() {
		...
		for (NetworkPacket hs : packetrecieved) {
			...
			List<HostPacket> pktlist = ((IBONetworkCloudletSpaceSharedScheduler) vm.getCloudletScheduler()).pktrecv
					.get(hs.pkt.sender);

			if (pktlist == null) {
				pktlist = new ArrayList<HostPacket>();
				((IBONetworkCloudletSpaceSharedScheduler) vm.getCloudletScheduler()).pktrecv.put(
						hs.pkt.sender,
						pktlist);
	...
	private void sendpackets() {
		...
		for (Vm vm : super.getVmList()) {
			for (Entry<Integer, List<HostPacket>> es : ((IBONetworkCloudletSpaceSharedScheduler) vm
		...
		for (NetworkPacket hs : packetTosendLocal) {
			...
			IBONetDatacenterBroker.totalTransferCost += hs.pkt.data * IBONetDatacenterBroker.transferCost[hs.pkt.sender][hs.pkt.reciever];
